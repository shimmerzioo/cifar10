...
def collect_training_details(variables):
    result_dict = {}
    for col in logging_columns_list:
        var = variables.get(col.strip(), None)
        if type(var) in (int, str):
            ...
            result_dict[col] = "{:0.4f}".format(var)
        else:
            assert var is None
            result_dict[col] = ""
    return result_dict
    ...
    for epoch in range(ceil(epochs)):
        ...
        for inputs, labels in train_loader:
            ...
            if verbose and is_warmup:
                ...
                training_details.append(collect_training_details(locals()))
                return
